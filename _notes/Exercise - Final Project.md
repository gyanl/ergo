---
date: 1-11-2025
date modified: 1-11-2025
feed: show
tag: exercise
title: Exercise - Final Project
---
### Project Ideas 
See [[Project Ideas]] for some possible project ideas, or propose your own.

### Proposal
*Due by 3 Nov*
*Extended deadline - Due by 13 Nov*

Describe the general problem area - what is the ergonomic problem you have identified? Why does it need to be redesigned? Describe your approach and what you will do over the next month? Break down week by week.

### Project Presentations
*Submissions due by 3 Dec*
*Class presentations on 5/6 Dec*

Describe the general problem area - what is the ergonomic problem you have identified? Why does it need to be redesigned? Describe your approach and what you will do over the next month? Break down week by week.




### Grading
The total grade for the ergonomics project is **20 points**, distributed across five key facets.

**Punctuality (2 points)** assesses timely submission of both the proposal and final deliverable, with one point allotted for each.

**Project Concept (3 points)** evaluates the impactfulness, clarity, and relevance of the chosen ergonomic problem or context, based on the initial proposal.

**Application of Principles (5 points)** focuses on how effectively ergonomic principles are applied to critique existing designs and identify key issues.

**Prototyping and Evaluation (5 points)** measures the ability to translate ergonomic insights into tangible design improvements and includes testing with users to assess usability or comfort.

Finally, **Documentation of Process and Outcome (5 points)** examines how clearly and effectively the project is communicated—covering research, prototypes, findings, and the overall coherence between research, design, and final outcomes.